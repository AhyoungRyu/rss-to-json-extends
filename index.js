...
module.exports = {
    ...
    parser: function(json){
        ...
        if (channel['dc:creator']) {
            rss.author = channel['dc:creator'];
        }
        
        // ahyoung: replace 'description' to 'content:encnded'
         if (channel['content:encoded']) {
             rss.description = channel['content:encoded'][0];
         }
         if (channel.link) {
         ...
         if (channel.item) {
           ...
           channel.item.forEach(function(val){
             ...
             obj.description = !util.isNullOrUndefined(val['content:encoded'])?val['content:encoded'][0]:'';
             obj.author = !util.isNullOrUndefined(val['dc:creator'])?val['dc:creator'][0]:'';
